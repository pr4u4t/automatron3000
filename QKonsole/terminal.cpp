...
int PopupCompleter::exec(QTextEdit* parent)
{
    ...
    QScreen* dsk = QGuiApplication::primaryScreen();
    QRect screenGeom = dsk->geometry(); //screenGeometry(dsk->screenNumber(this));
...
QTerminal::QTerminal(QWidget* parent, const QString& welcomeText)
    ...
    promptLength(0), promptParagraph(0), isLocked(false)
{
    ...
    // see function QTerminal::contextMenuEvent(...)
...
void QTerminal::setPrompt(const QString& newPrompt, bool display)
{
    prompt = newPrompt;
...
void QTerminal::setFont(const QFont& f) {
...
QStringList QTerminal::suggestCommand(const QString&, QString& prefix)
{
    prefix = "";
    return QStringList();
}
...
void QTerminal::handleTabKeyPress()
{
...
void QTerminal::handleReturnKeyPress()
{
...
bool QTerminal::handleBackspaceKeyPress()
{
    QTextCursor cur = textCursor();
...
void QTerminal::handleUpKeyPress()
{
    if (history.count())
    {
        QString command = getCurrentCommand();
...
void QTerminal::handleDownKeyPress()
{
    if (history.count())
    {
        QString command = getCurrentCommand();
...
void QTerminal::setHome(bool select)
{
...
QString QTerminal::getCurrentCommand()
{
...
void QTerminal::replaceCurrentCommand(const QString& newCommand)
{
    QTextCursor cursor = textCursor();
...
bool QTerminal::isCommandComplete(const QString&)
{
    return true;
}
...
bool QTerminal::isInEditionZone()
{
...
//or in the next lines (in case of multi-line mode)
bool QTerminal::isInEditionZone(const int& pos)
{
    QTextCursor cur = textCursor();
    cur.setPosition(pos);
...
bool QTerminal::isSelectionInEditionZone()
{
...
QString QTerminal::addCommandToHistory(const QString& command)
{
...
void QTerminal::printCommandExecutionResults(const QString& result, ResultType type)
{
...
void QTerminal::mousePressEvent(QMouseEvent* event)
{
...
void QTerminal::dragMoveEvent(QDragMoveEvent* event)
{
...
void QTerminal::correctPathName(QString& pathName)
{
