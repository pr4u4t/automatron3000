...
SettingsDialog::SettingsDialog(QWidget* parent, Loader* loader, const QString& settingsPath)
    : SettingsMdi(parent, new ChartSettings(Settings::get(), settingsPath), settingsPath)
    , m_ui(new Ui::SettingsDialog)
    , m_intValidator(new QIntValidator(0, 4000000, this)) {
    setup();
}

SettingsDialog::SettingsDialog(QWidget* parent, const Chart* chart)
    : SettingsMdi(parent, new ChartSettings(*(chart->settings<ChartSettings>())), chart->settingsPath())
    , m_ui(new Ui::SettingsDialog)
    , m_intValidator(new QIntValidator(0, 4000000, this)) {
    setup();
}
...
void SettingsDialog::fillFromSettings() {
    ...
    ChartSettings* setts = settings<ChartSettings>();
    m_ui->titleEdit->setText(setts->title());
}

SettingsDialog::operator ChartSettings() const {
    ChartSettings ret;
...
void SettingsDialog::updateSettings() {
    ...
    ChartSettings newSettings = *this;
    ChartSettings* setts = settings<ChartSettings>();
